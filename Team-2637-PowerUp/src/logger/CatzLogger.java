...
public class CatzLogger {
	...
	public void add(String object, String operation, int prior, double time) {
		...
		if (canLog() && CatzLogger.logLevel == 5) {
			if(counter % 1000 == 0) {
				CatzMessage m = new CatzMessage(object, operation, prior, time);
				toLog.add(m);
			}
		}else if(canLog()){
			CatzMessage m = new CatzMessage(object, operation, prior, time);
	...
	// increment counter at the end of a loop
	public static void incrementLog() {
		counter++;
	}
